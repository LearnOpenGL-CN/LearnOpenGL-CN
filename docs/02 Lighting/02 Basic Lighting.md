...
现在我们已经有了所有需要的变量，可以计算高光强度了。首先，我们定义一个镜面强度(Specular Intensity)值，使得镜面高光有一个中等亮度的颜色，从而让它不会影响过大。
/
```c++
float specularStrength = 0.5;
```

如果我们把它设置为1.0f，我们会得到一个非常亮的镜面光分量，这对于一个珊瑚色的立方体来说有点太多了。下一节教程中我们会讨论如何合理设置这些光照强度，以及它们是如何影响物体的。下一步，我们计算视线方向向量，以及相应的沿着法线轴的反射向量：
...
需要注意的是我们对`lightDir`向量进行了取反。`reflect`函数期望第一个向量**从**光源指向片段位置，但是`lightDir`的指向正好相反：从片段**指向**光源（这取决于先前我们计算`lightDir`向量时做减法的顺序）。为了确保我们得到正确的`reflect`向量，我们先对`lightDir`向量取反来反转它方向。第二个参数应是一个法向量，所以我们传入标准化的`norm`向量。

剩下要做的是计算镜面光照分量。通过下面的代码完成：
...
我们先计算视线方向与反射方向的点积（并确保它不是负值），然后取它的32次幂。这个32是高光的<def>反光度</def>(Shininess)。一个物体的反光度越高，它就能越好地反射光，而不是四面八方散射，于是高光点就会越小。在下面的图片中你可以看到不同反光度值的视觉效果影响：

![](../img/02/02/basic_lighting_specular_shininess.png)

我们不希望镜面成分过于显眼，所以我们把指数选定为32。剩下的最后一件事情是把它与环境光分量和漫反射分量加到一起，再把结果乘以物体的颜色：
...
我们现在计算了冯氏光照模型的所有部分。基于你的视角，你应该看到类似这样的画面：
...
!!! Important
    ...
    在光照着色器的早期，开发者曾经在顶点着色器中实现冯氏光照模型。在顶点着色器中做光照的优势是，顶点数目要比片段数目少得多，因此计算会高效得多，所以（开销大的）光照计算频率会更低。然而，顶点着色器的颜色值结果只是那个顶点自身的光照颜色值结果，它周围片段的颜色值则是由数个光照颜色值插值得到的。结果就是这种光照看起来不会非常真实，除非使用了大量顶点。
...
至此你应该开始看到着色器有多么强大了。只用很少的信息，着色器就能计算出光照如何影响到所有物体的片段的颜色。[下一节](03 Materials.md)中，我们会更深入的探索我们能用光照模型做些什么。

## 练习

- 目前的光源是个有点无聊的不会移动的静态光源，尝试使用<fun>sin</fun>或<fun>cos</fun>函数让光源在场景中移动。观察光照随时间的改变能让你更好地理解冯氏光照模型。[参考解答](https://learnopengl.com/code_viewer.php?code=lighting/basic_lighting-exercise1)。
- 使用不同的环境光、漫反射和镜面强度，观察它们怎么是影响光照效果的。同样，对镜面光照的反光度因子做些实验。尝试理解为什么某一个值能够产生某一种视觉输出。
- 在观察空间（而不是世界空间）中计算冯氏光照：[参考解答](https://learnopengl.com/code_viewer.php?code=lighting/basic_lighting-exercise2)。
- 实现Gouraud着色（而不是冯氏着色）。如果你做对了话，立方体的光照应该会[看起来有点不对劲](../img/02/02/basic_lighting_exercise3.png)，尝试找到原因，为什么它会看起来这么奇怪：[参考解答](https://learnopengl.com/code_viewer.php?code=lighting/basic_lighting-exercise3)。
